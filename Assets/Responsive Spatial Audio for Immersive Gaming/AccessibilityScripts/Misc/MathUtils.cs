...
using UnityEngine;
    ...
    public static class MathUtils
    {
        ...
        public static bool TestPlanesAABB(Plane[] planes, int planeMask, Collider c, out bool entirelyInside)
        {
        bool counter = false;
        ...
        Bounds bounds = c.bounds;    
            ...
            while (mask <= planeMask)
            {
                ...
                if ((uint)(planeMask & mask) != 0)
                {
                    ...
                    //float radius = Vector3.Dot(new Vector3(boundsExtent.x,0,boundsExtent.z), n);
                    ...
                    if (counter == true && planeIndex == 0)
                    {
                        Debug.DrawRay(boundsCenter, boundsExtent, Color.cyan, 10.0f);
                        Debug.DrawRay(boundsCenter, n, Color.cyan, 10.0f);
                        Plane pp = p;
                        //Debug.Log("distance: " + distance + " radius: " + radius);
                        //Debug.Log("disatnce from boundscentre to near plane: " + Vector3.Distance(boundsCenter,))
                        counter = true;
                    }
               

                    if (distance + radius < 0) // aabb is either entirely on the anti-normal side or intersecting with plane
                    {
                       // behind clip plane
                    ...
                    if (distance > radius)  // aabb is entirely on the normal side of the plane
                    {
                        entirelyInsideCount++;
                    }
                }
